...
public class FlexibleRatingBar extends AppCompatRatingBar {
    ...
    private void drawHighRating(Canvas canvas) {
    ...
    private void drawLowRating(Canvas canvas) {
        ...
        for (int i = 0; i < getNumStars(); ++i) {
            ...
            path.computeBounds(rectangle, true);
            canvas.save();
            canvas.translate((i + .5F) * getWidth() / getNumStars() - rectangle.centerX(), getHeight() / 2 - rectangle.centerY());
//            path.offset((i + .5F) * getWidth() / getNumStars() - rectangle.centerX(), getHeight() / 2 - rectangle.centerY());
            float rating = getRating();
            if (rating >= i + 1) {
                BitmapShader shaderFill = updateShader(colorFillOn, colorFillOn);
                BitmapShader shaderFillPressed = updateShader(colorFillPressedOn, colorFillPressedOn);
                paintInside.setShader(isPressed() ? shaderFillPressed : shaderFill);
                paintInside.setColor(isPressed() ? colorFillPressedOn : colorFillOn);
                canvas.drawPath(path, paintInside);
            } else if (rating < i + 1 && rating > i) {
                BitmapShader shaderFill = updateShader(rectangle, rating - i, colorFillOn, colorFillOff);
                BitmapShader shaderFillPressed = updateShader(rectangle, rating - i, colorFillPressedOn, colorFillPressedOff);
                paintInside.setShader(isPressed() ? shaderFillPressed : shaderFill);
                canvas.drawPath(path, paintInside);
            }
    ...
    protected void onDraw(Canvas canvas) {
        canvas.clipRect(0, 0, viewWidth, getMeasuredHeight());
        if (Build.VERSION.SDK_INT == Build.VERSION_CODES.JELLY_BEAN) {
            drawLowRating(canvas);
        } else {
            drawHighRating(canvas);
        }

    }


    private BitmapShader updateShader(RectF rectF, float progress, int colorOn, int colorOff) {

        //Bitmap of width 0 will cause a crash. Make sure it's a positive number.
        progress = Float.parseFloat(String.format("%.1f", progress));
        int ratingWidth = (int) (rectF.width() * progress);
        ...
        if (ratingWidth <= 0 || getWidth() - ratingWidth <= 0) {
            ...
            Bitmap colorLeft = Bitmap.createBitmap(ratingWidth, (int) rectF.height(), Bitmap.Config.ARGB_8888);
            colorLeft.eraseColor(colorOn);

            Bitmap colorRight = Bitmap.createBitmap((int) Math.ceil(rectF.width() - ratingWidth + 2), (int) rectF.height(), Bitmap.Config.ARGB_8888);
    ...
    private void getXmlAttrs(Context context, AttributeSet attrs) {
        ...
        try {
            ...
            isHide = a.getBoolean(R.styleable.FlexibleRatingBar_isHide, false);
            strokeWidth = (int) a.getDimension(R.styleable.FlexibleRatingBar_flexibleStrokeWidth, -1);
        } finally {
            a.recycle();
        }
    }
