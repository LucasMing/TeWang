...
public class AudioRecorder {
    ...
    public void startRecord() {
        ...
        mExecutorService.execute(() -> writeDataTOFile(finalFileName));
    }
    ...
    public void writeDataTOFile(String currentFileName) {
        ...
        while (status == Status.STATUS_START) {
            AudioChunk audioChunk = new AudioChunk.Bytes(new byte[bufferSizeInBytes]);
            readsize = audioRecord.read(audioChunk.toBytes(), 0, bufferSizeInBytes);
            if (AudioRecord.ERROR_INVALID_OPERATION != readsize && fos != null) {
                try {

                    fos.write(audioChunk.toBytes());
                    if (listener != null) {
                        //用于拓展业务
                        listener.onRecording(audioChunk.toBytes(), 0, audioChunk.toBytes().length,audioChunk);
                    }
                } catch (IOException e) {
                    Log.e("AudioRecorder", e.getMessage());
                }
    ...
    public String createWav() {
        ...
        if (filesName.size() > 0) {
            ...
            PcmToWav.mergePCMFilesToWAVFile(filePaths, FileUtils.getWavFileAbsolutePath(fileName), false);
        } else {
            PcmToWav.makePCMFileToWAVFile(FileUtils.getPcmFileAbsolutePath(fileName), FileUtils.getWavFileAbsolutePath(fileName), false);
        }


        return FileUtils.getWavFile(fileName);
    }
